...
class CurrentUser:
    ...
    def __init__(self, user_id: str, tenant_id: str, email: str, roles: list = None):
...
async def get_current_user(
    ...
    """
    # Fallback para autenticação via headers customizados (Dev/Mock)
    # Aceita se tiver pelo menos X-User-ID (X-Organization-ID pode ser vazio para super-admin)
    if not authorization and x_user_id:
        try:
            return CurrentUser(
                user_id=x_user_id,
                tenant_id=x_org_id or "default-org-001", # Fallback para org default
                email="dev@example.com", # Email fictício para dev
                roles=["admin"] # Role admin por padrão para dev
            )
        except ValueError:
             raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="IDs de usuário ou organização inválidos nos headers"
            )
